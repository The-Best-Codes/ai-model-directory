...
export const fastrouterProvider: ProviderDefinition = {
  name: "fastrouter",
  outputDirectory: "data/providers/fastrouter/models",
  async fetchModels(progress) {
    progress?.beginPhase("fetching", 1);

    const response = await fetchJson("https://go.fastrouter.ai/api/v1/models", {
      schema: responseSchema,
      headers: withBearerToken(process.env.FASTROUTER_API_KEY),
      label: "FastRouter API error",
    });

    progress?.tick(
      `go.fastrouter.ai/api/v1/models (${response.data.length})`,
      true,
    );
