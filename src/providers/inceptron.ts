...
export const inceptronProvider: ProviderDefinition = {
  name: "inceptron",
  outputDirectory: "data/providers/inceptron/models",
  async fetchModels(progress) {
    progress?.beginPhase("fetching", 1);

    const response = await fetchJson("https://api.inceptron.io/v1/models", {
      schema: responseSchema,
      headers: withBearerToken(process.env.INCEPTRON_API_KEY),
      label: "Inceptron API error",
    });

    progress?.tick(
      `api.inceptron.io/v1/models (${response.data.length})`,
      true,
    );
