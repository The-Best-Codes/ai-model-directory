...
export function ModelTable({
  ...
  const { modelRows, modelCount, providerCount } = React.useMemo(
    () => buildRows(directory, providerId),
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3 tabular-nums">
        ...
        <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            <span className="hidden sm:inline">
              ...
              {providerId
                ? ` from ${providerCount} ${providerCount === 1 ? "provider" : "providers"}`
                : ` from ${providerCount} providers`}
            </span>
          </p>
        </div>
...
function buildRows(
  ...
  const providerEntries = Object.values(directory)
